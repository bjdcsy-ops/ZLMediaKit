...
## NtpStamp 长期累计精度（2026-09-11）

旧 `NtpStamp` 对每次 RTP 增量使用 float 换算，再截断为整数微秒并更新锚点。
16 kHz AAC 每个 1024 样本增量因此从 64000 µs 变成 63999 µs；没有源 SR
重新校准时，每小时少计 56.25 ms，21 小时约 1.181 秒。

修复使用 64 位整数换算，并把不足一微秒的除法余数带入下一次推进，兼顾
44.1/48 kHz 等无法整除微秒的步长。连续时间由累计 RTP ticks 决定，最后才投影
到公开接口的整数毫秒。合成测试直接推进媒体时间，不等待真实的 21 小时。

| 事件 | 时间映射行为 |
| --- | --- |
| 正常推进或正向 RTP 回绕 | 累计整数微秒并保留余数 |
| 重复 RTP 时间戳 | 返回当前值，不重复累计 |
| 普通乱序或晚到的回绕前 RTP | 按当前锚点及余数计算历史时间，不改变推进状态 |
| 有效 SR | 立即采用新锚点，清除旧余数；无效 SR 仍按原规则忽略 |
| 异常大幅跳变 | 保持原整数微秒时间，重新锚定 RTP 并清除余数 |
| RTP clock rate 变化 | 保留整数微秒锚点，清除旧采样率的余数；同戳查询也执行此规则 |
| 正常方向增量恰好 3 秒 | 进入既有异常处理；旧 float 在 8/16 kHz 下误算为略小于 3 秒 |

公开方法签名和配置项不变，`NtpStamp` 增加私有状态，需要重新编译使用它的 C++
组件。原有回绕识别条件不变；本项不调整 SR 校时策略、操作系统校时后的绝对时间
基准或 `rtsp_input_clock` 模式，也不代表 NVR 静音已修复。

在已配置的 Linux ARM64 构建环境中执行：

```bash
cmake -S . -B .build/linux-arm64/zlmediakit -DENABLE_TESTS=ON
cmake --build .build/linux-arm64/zlmediakit --target test_rtp_ntp_clock -j6
# 新增长期精度与状态边界；每个参考值来自绝对累计 ticks。
release/linux/Release/test_rtp_ntp_clock --verify-ntp-precision
# 默认测试也包含以上用例，以及原有 receiver/decoder/track/muxer 兼容检查。
release/linux/Release/test_rtp_ntp_clock
# 原始现场 SR 重锚验收独立保留，不因本次数值修复放宽其失败门槛。
release/linux/Release/test_rtp_ntp_clock --verify-sr-reanchor
```

本次 Debian 11 ARM64 验证结果（修复前基线为 `f08928c6`）：

| 验证 | 结果 |
| --- | --- |
| 旧实现负对照 | 原有 12 组默认测试通过；新增 6 组中 4 组失败，16 kHz 首步实得 63 ms、应为 64 ms |
| 新增精度专项 | 6 组通过；8/16/32/44.1/48/64 kHz 各约 21 小时，全部观测点对绝对样本时长的毫秒偏差为 0 |
| 默认 RTP/NTP 回归 | 18 组通过 |
| G711 / AAC / RTSP 音频 / RTSP muxer | 全部通过；分别含 96 组、5252 个捕获加入位置、14 组、20 组 |
| 可选输入时钟 / decoder-mux 链 / player-proxy 集成 | 三个测试目标均通过 |
| 独立 SR 重锚验收 | 修复前后均退出 1，失败用例集合仍为原有 8 项，未放宽断言 |

构建和测试日志保存在本地 `.build/field-tests/20260911/ntp-precision/`。
以上为合成输入与已有 fixture 的回归结果，未部署生产，未进行 NVR 听音验收。

按 IPC 设置补充的采样率矩阵已在相同 ARM64 环境通过，专项及默认 18 组测试均
重新运行。每步为 1024 ticks，逐帧与绝对样本计数换算的毫秒值严格相等：

| 采样率 | 推进帧数 | 合成媒体时长 | 累计偏差 | 自然 RTP 回绕 |
| --- | ---: | --- | ---: | ---: |
| 8 kHz | 590625 | 21 小时 | 0 ms | 0 次 |
| 16 kHz | 1181250 | 21 小时 | 0 ms | 0 次 |
| 32 kHz | 2362500 | 21 小时 | 0 ms | 0 次 |
| 48 kHz | 3543750 | 21 小时 | 0 ms | 0 次 |
| 64 kHz | 4725000 | 21 小时 | 0 ms | 1 次 |

44.1 kHz 分数微秒用例继续保留；整帧取整后的媒体时长比 21 小时少约 7.256 ms，
这不是累计漂移。新增采样率也覆盖主动跨回绕、重复、乱序及双向 3 秒 ±1 tick
边界。补测日志为上述目录中的 `ipc-rates-precision.log` 和 `ipc-rates-default.log`。
